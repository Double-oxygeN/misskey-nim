...
proc postJson(client: MockedHttpClient; uri: Uri; body: JsonNode; headers: openArray[HeaderField]): JsonResponse =
  ...
  check body["withUnreads"].getBool()
  check body["sinceId"].getStr() == "fghij67890"
  check body["untilId"].getStr() == "klmno12345"
  result.code = Http200
...
let
  ...
  announcemts = client.announcements(withUnreads = true, sinceId = "fghij67890", untilId = "klmno12345")
